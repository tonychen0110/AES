...
public class Helper {
	...
	static byte[][] toMatrix(byte[] array) {
		...
		for (int i = 0, col = 0; col < matrixLength; col++) {
	...
	//Addition in the Galois  Field is xor of two numbers
	...
	static byte galoisMultiply(byte a, byte b) {
		...
		for (int i = 0; i < 8; i++) {
			...
			//Rotating a one bit to the left discard
			//high bit and make low bit equal to 0
			a <<= 1;
			...
			//Rotating b one bit to the right discard
			//low bit and make high bit equal to 0
			b >>= 1;
		}
